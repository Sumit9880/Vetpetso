...
import SemenCompanyDrawer from '../components/SemenCompanyDrawer';
import Pagination from '../components/Pagination';
import Loader from '../components/Loader';

function SemenCompany() {
    const [semenCompanies, setSemenCompanies] = useState([]);
    ...
    const getData = useCallback(async () => {
        ...
        try {
            ...
            const res = await apiPost("api/semenCompany/get", {
                filter,
                pageSize,
    ...
    return (
        <div className="container mx-auto p-3 bg-gray-100 rounded h-full">
            <div className='flex justify-between my-2 items-center'>
                <h1 className="text-2xl font-bold mb-2 text-start">Semen Companies</h1>
                <div className="flex justify-end mb-2">
                    ...
                    <SemenCompanyDrawer isOpen={isDrawerOpen} onClose={handleCloseDrawer} data={semenCompany} />
                </div>
            </div>
            <div className="overflow-x-auto" style={{ height: 'calc(100vh - 214px)' }}>
                ...
                <table className="table-auto w-full border-collapse border border-gray-400 rounded-lg">
                    ...
                    <tbody>
                        {semenCompanies?.map(semenCompany => (
                            <tr key={semenCompany.ID} className="bg-white">
                                <td className="px-2 border border-gray-200 text-center">{semenCompany.ID}</td>
                                <td className="px-2 border border-gray-200">{semenCompany.NAME}</td>
                                <td className={`px-2 border border-gray-200 text-center${semenCompany.STATUS ? " text-green-500" : " text-red-500"}`}>{semenCompany.STATUS ? "On" : "Off"}</td>
                                <td className="px-2 border border-gray-200">{semenCompany.TYPE}</td>
                                <td className="px-2 border border-gray-200 text-center">
                                    <button className="py-2 text-center" onClick={() => handleOpenDrawer(semenCompany)}><LiaEditSolid className="text-blue-500 hover:text-blue-700 h-5 w-5" /></button>
                                </td>
                            </tr>
                        ))}
                    </tbody>
                </table>
                {semenCompanies.length > 0 || loader ? null :
