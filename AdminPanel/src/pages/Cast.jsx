...
function Cast() {
    ...
    const getData = useCallback(async () => {
        ...
        try {
            ...
            const res = await apiPost("api/cast/get", {
                filter,
                pageSize,
    ...
    return (
        <div className="container mx-auto p-3 bg-gray-50 rounded h-full">
            <div className='flex justify-between my-2 items-center'>
                <h1 className="text-2xl font-bold mb-2 text-start">Casts</h1>
                <div className="flex justify-end mb-2">
                    ...
                    <CastDrawer isOpen={isDrawerOpen} onClose={handleCloseDrawer} data={cast} />
                </div>
            </div>
                    ...
                    <tbody>
                        {casts?.map(cast => (
                            <tr key={cast.ID} className="bg-white">
                                <td className="px-2 border border-gray-200 text-center">{cast.ID}</td>
                                <td className="px-2 border border-gray-200">{cast.NAME}</td>
                                <td className={`px-2 border border-gray-200 text-center${cast.STATUS ? " text-green-500" : " text-red-500"}`}>{cast.STATUS ? "On" : "Off"}</td>
                                <td className="px-2 border border-gray-200 text-center">
                                    <button className="py-2 text-center" onClick={() => handleOpenDrawer(cast)}><LiaEditSolid className="text-blue-500 hover:text-blue-700 h-5 w-5" /></button>
                                </td>
                            </tr>
                        ))}
                    </tbody>
                </table>
                {casts.length > 0 || loader ? null :
