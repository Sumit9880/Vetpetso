...
function reqData(req) {
    var data = {
        ...
        ANIMAL_TYPE_ID: req.body.ANIMAL_TYPE_ID
    }
    return data;
}
...
exports.validate = function () {
    return [
        ...
        body('ANIMAL_TYPE_ID', ' parameter missing').isInt().exists(),
        body('ID').optional(),
    ]
}
...
exports.get = (req, res) => {
    ...
    try {
        dm.runQuery('select count(*) as cnt from view_animal_breed_master where 1 ' + countCriteria, req, (error, results1) => {
            if (error) {
                console.error(error);
                res.send({
                    "code": 400,
                ...
                dm.runQuery('select * from view_animal_breed_master where 1 ' + criteria, req, (error, results) => {
                    if (error) {
                        console.error(error);
                        res.send({
                            "code": 400,
...
exports.create = (req, res) => {
    ...
    if (!errors.isEmpty()) {
        ...
        try {
            dm.runDataQuery('INSERT INTO animal_breed_master SET ?', data, req, (error, results) => {
                if (error) {
                    console.error(error);
                    res.send({
                        "code": 400,
...
exports.update = (req, res) => {
    ...
    if (!errors.isEmpty()) {
        ...
        try {
            dm.runDataQuery(`UPDATE animal_breed_master SET ${setData} LASTUPDATED = ? where ID = ?;`, recordData, req, (error, results) => {
                if (error) {
                    console.error(error);
                    res.send({
                        "code": 400,
