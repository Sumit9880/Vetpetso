...
exports.validate = function () {
    return [
        ...
        body('DELIVERY_DATE', 'DELIVERY_DATE parameter missing').exists().isISO8601(),
        body('IS_PREGNANT', 'IS_PREGNANT parameter missing').exists(),
        body('MILK_PRODUCTION', 'MILK_PRODUCTION parameter missing').exists(),
        body('ID').optional(),
    ]
}
...
exports.create = (req, res) => {
    ...
    if (!errors.isEmpty()) {
        ...
        try {
            dm.runDataQuery('INSERT INTO ai_details SET ?', data, req, (error, results) => {
                if (error) {
                    console.error(error);
                    res.send({
                        "code": 400,
...
exports.update = (req, res) => {
    ...
    if (!errors.isEmpty()) {
        ...
        try {
            dm.runDataQuery(`UPDATE ai_details SET ${setData} LASTUPDATED = ? where ID = ?;`, recordData, req, (error, results) => {
                if (error) {
                    console.error(error);
                    res.send({
                        "code": 400,
